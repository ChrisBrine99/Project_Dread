...
// Values for the bits within this object's flags variable that are used by it and any children that may inherit from it.
...
// Inherit all functions and variables from the parent object and its parent objects. Then, toggle the item to be visible and active so it
...
// Variables for determining what item this object represents within the world. The "worldItemID" is used to reference a persistent struct 
// of the item's state between rooms. Meanwhile, the other three variables are for the item's ID value, the total number of the item (Or its
// ammunition in the case of ranged weapons), and the starting durability of the item.
...
on_player_interact = function(_delta){
	...
	// Get the reference to the item's struct containing all its data, and then grab the type of the item from that data since it is utilized
	// throughout this interaction function.
	var _itemData = global.itemData[? itemName];
	var _itemType = _itemData.typeID;
	
	// Only a portion of the available amount could be added to the player's item inventory. The text shown in the textbox will say how much 
	// was picked up; along with flavor text letting the player know that they have no room left in their item inventory.
	...
	// Grab the item's stack limit from within the item struct's data, and set it to one if the item happens to be either an equipable item 
	// or a weapon so the textbox doesn't display its quantity on pick up.
	...
	// to reflect that the item is now considered collected.

...
// The list of flag bits being utilized by the cutscene collider currently.Each macro is their respective position within the variable (Ex. 
// 0x80000000 is the 32nd bit, 0x00400000 is the 23rd bit, and so on).
...
// The bit within the event flag buffer that a given cutscene collider instance is tied to. Normally, this flag being the state this object is
// looking for will result in it being destroyed so the scene cannot replay, but if the NEVER_DESTROY flag is set, the cutscene is replayable.
flagID		= EVENT_ID_INVALID;

// A list containing the actions that will be executed upon activating the cutscene through the player object colliding with this object.

...
// so a proper state switch doesn't occur in the middle of updating.
...
// Perform code similar to the loop above does for existing menu objects, but for singleton objects that utilize state machines but don't 
// need full end_step events like dynamic and static entities do.
...
// Returning control back to the keyboard if a gamepad was previously the active input method but a key on the keyboard was pressed by the 
// user during the current frame.
...
// Looping through all buttons for the gamepad to see if any of them have been pressed by the user. If so, control is moved over to the 
// controller that the input was read from.
...
// Check the primary stick to see if any input is detected from it. If so, move input detection over to the connected gamepad.
if (gamepad_axis_value(_gamepad, gp_axislh) != 0.0 || gamepad_axis_value(_gamepad, gp_axislv) != 0.0){
	global.flags = global.flags | GAME_FLAG_GAMEPAD_ACTIVE;
	return;
}
	
// Finally, the secondary stick is checked (If one exists) for input. If input is found, control is moved over to the connected gamepad.

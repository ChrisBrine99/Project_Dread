...
// Removes all item inventory structs as they aren't the standard structs that are automatically maintained by the global.struct data 
// structure. In very rare cases, the item inventory will not have been initialized  before this event executes. In that case, this entire 
// chunk of code is skipped since no cleanup is needed.
...
// Loop through all world item structs that may currently exist. Then, clear and destroy the data structure manages all world items.
...
// Clear out the references to structs found within these struct-specific lists. They will remain alive until all structs have been destroyed.
// Once that occurs, these lists will be destroyed.
ds_list_clear(global.lights);
ds_list_clear(global.menus);

// Remove all existing struct instances from memory by deleting their references stores within the global struct management list. Then, that
// list itself is destroyed to clear it from memory as well.
...
// After structs have been cleaned up, the lists that store specific struct instances will finaly be destroyed to avoid issues with clean up 
// of various different objects.
...
// Delete the grid that stores entities and their current y position so they can be drawn from the top of the screen to the bottom in order.
ds_grid_destroy(global.sortOrder);

// Clean up the list that is a temporary storage for the states of entities prior to being paused whenever they are paused. Primarily used 
// during cutscenes so states can be used for things like moving entities.
...
// Finally, remove and clear out any data structures or dynamic memory that was allocated by the game manager itself for handling various 
// logic that occurs on a global scale within the game.

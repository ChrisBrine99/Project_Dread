...
// Check when to end the room transition process. This can only happen if the current room matches the target. Otherwise, the game will 
// ignore this branch even if the screen fade's alpha level is zero.
...
// Check for when the screen fade is completely opaque. At this point, the room can be switched without the player noticing the change 
// occurring. After this, the screen fade will be signaled to begin fadeing out so the transition can complete.
if (_fadeAlpha == 1.0 && room != targetRoom){
	// First, load in the target room.
	room_goto(targetRoom);
	
	// Begin looping through all instances that need to warp to the target room. They will have their positions updates to match the required 
	// coordinates and their persistence flag will be reset to what it was prior to the warp event occurring. Then, this map is cleared of 
	// all elements.
	...
	// Finally, signal to the screen fade effect that it can begin fading back out which will then end the warp event and bring the game 
	// back into the control of the player's actions.

// Call the camera's room start evernt which sets up the viewport of the newly loaded room to match the camera's viewport properties while 
// also enabling said viewport in the room.
with(CAMERA) { room_start_event(); }

// Check for any dynamic items that may exist within the newly loaded room by looping through the keys stored within the global 
...
for (var i = 0; i < _length; i++){
	// Check if the data for the current item's key still exists. If not the key is removed from the list so it won't be created again.
...
// Get the room's collision object layer so its visibility can be set to false as it is kept true within the editor so collision bounds are 
// known at all times.
var _layerCollision = layer_get_id("Collision");
if (_layerCollision != -1)
	layer_set_visible(_layerCollision, false);

// Much like the collision object layer, the floor materials tile layer will be set to invisible since it will remain visible in the editor 
// so material tiles are known while editing the given area.
...
// Finally, get the ID for the floor material tile layer if the room has one, and assign the ID so the player can utilize it to handle their
// step sound effect logiic when required.
with(PLAYER){

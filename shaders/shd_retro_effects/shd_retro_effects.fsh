...
void main(void) {
	...
	float _noise = 0.0;
	if (ditheringActive){
		float	_screenY	= floor(v_vTexcoord.y * viewportSize.y);		// Scale the screen coordinates to match the application surface's dimensions.
		float	_screenX	= floor(v_vTexcoord.x * viewportSize.x);

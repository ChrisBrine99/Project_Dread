...
void main(void) {	
	...
	vec3 _texColor = vColor * texture2D(gm_BaseTexture, vTexcoord).rgb;
	...
	if (!quantizationActive && !ditheringActive && !scanlinesActive){
		gl_FragColor = vec4(_texColor, 1.0);
		return;
	}
	...
	float _noise = 0.0;
	if (ditheringActive){
		float	_screenX	= floor(vTexcoord.x * viewportSize.x);			// Scale the screen coordinates to match the application surface's dimensions.
		float	_screenY	= floor(vTexcoord.y * viewportSize.y);			
		...
		if (!quantizationActive) // Applying dithering if quantization happens to be inactive.
			_texColor = clamp(floor(_texColor * 255.0 + _noise), vec3(0.0), vec3(255.0)) / 255.0;
	}
	...
	if (quantizationActive){
		_texColor = clamp(floor(_texColor * quantizeLevel + _noise), vec3(0.0), vec3(quantizeLevel));
		_texColor = _texColor / quantizeLevel;
	}
	...
	if (scanlinesActive && mod(floor(vPosition.y), 2.0) != 0.0)
		_texColor = _texColor * scanlineFactor;
    gl_FragColor = vec4(_texColor, 1.0);
}

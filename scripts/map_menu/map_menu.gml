...
function str_map_menu(_index) : str_base_menu(_index) constructor {
	// Stores a reference to the control icon group that displays input information for the map menu as well as the group that is normally 
	// managed by the inventory since it holds info about cursor movement and that needs to be updated when a map is selected for viewing/
	// moving around it.
	...
	draw_gui_event = function(_xPos, _yPos, _wView, _hView){
		...
		draw_text_shadow(_xPos + (_wView >> 1), _yPos + (_hView >> 1), "MAPS", COLOR_WHITE, alpha);

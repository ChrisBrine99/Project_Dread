...
// are stored in a seperate map structure.
global.worldItems		= ds_map_create();

// Two important values for how dynamic items will be created within the game. The list stores the keys for each dynamic item that is found 
// within global.worldItems, and the number is increment for every dynamic item created by the player. The list is used to check on room 
// start for any items that need to be created.
global.dynamicItemKeys	= ds_list_create();
global.nextDynamicKey	= 1000000;

// A list containing the keys for items that have been collected by the player. This prevents the item from being collected multiple times 
// since the game's rooms/areas aren't flagged as persistent.
...
// Macros for keys that show up in multiple sections of the unprocessed global item data structure contents. Each will be placed into a 
// variable within the struct found in the processed item data structure list.
#macro	KEY_NAME						"Name"
...
// Macros to explain what each index in the _splitStr variable represent during the process of parsing an item ID and its quantity cost out 
// of the combo data being loaded in.
...
function load_item_data(_filename){
	...
	// Try and get a reference to the loaded in combination data. If that data cannot be found, the item data is malformed in some way, so 
	// the loading process is aborted before anything can be loaded.
	...
	for (var i = 0; i < _length; i++){
		...
		// Parse the first item's ID and its potential additional cost data. By default, the cost will be 1, but a unique value may be 
		// parsed if the item ID is stored as a string instead of a number.
		...
		// Parse the second item's ID and its potential additional cost data in the same way that the first item has its information parsed.
	...
	while(!is_undefined(_curSection)){
		// If the current section is the chunk that contains the game's combination data, skip over it so it isn't loaded using the logic 
		// below, as it is not compatible with it like the rest of the data is.
		...
		// Find the first element in the section that is being parsed. Then, loop through that section until there are no more items in the 
		// sections to parse (The loop exits early if invalid data is found).
		...
		while(!is_undefined(_curItemID)){
			...
			// Load the item into the correct format that the game expects; parsing out its combo data from a string into structs containing 
			// the requirements and outcomes for a combo.
...
function load_item(_section, _itemKey, _itemIndex, _data, _useFunctions){
	...
	// Create the struct and add it to the global item data structure (This data struct is responsible for deleting the struct once it is no
	// longer needed). Also add that reference to a list at the position that matches the id value of the item.
	...
	switch(_section){
		case KEY_WEAPONS: // Parse through the data of a weapon item.
			with(_itemStructRef){
				...
				// Begin adding parameters that are unique to a weapon type item (Some are also the same as what an ammo-type item will have 
				// defined for it, and durability is the same as what is also defined in an equipable-type item).
				...
				// Set the flag bits utilized by weapon-type items based on the values parsed through the item data for the weapon in 
				// question; offseting them to match the bit's position in the variable.
				...
				// Determine which type of weapon this is when equipped by checking if the item is thrown or not. If it is thrown, it will 
				// always be considered a subweapon.
		...
		case KEY_AMMO: // Parse through the data of an ammo item.
			with(_itemStructRef){
				...
				// Begin adding parameters to the default item struct so that it can contain all the values needed for an ammo-type item 
				// (All are the same as variables found in weapon-type items).
		...
		case KEY_CONSUMABLE: // Parse through the data of a consumable item.
			with(_itemStructRef){
				...
				// Begin adding parameters to the default item struct so that it can contain all the values needed for a consumable-type item.
				...
				// Set the flag bits utilized by consumable-type items based on the values parsed through the item data for the consumable 
				// in question; offseting them to match the bit's position in the variable's numerical value.
...
/// @description 
/// Initializes a special type of world item element within the world item data structure. These are items that were collected previously 
...
#endregion Crafting Data Functions
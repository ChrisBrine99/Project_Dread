...
// Macros for checking the current state of the flag bits for the inventory menu. They are used by the active submenu to check if they can 
// or cannot perform certain actions to the inventory.
...
// Various constants relating to the options shown by the inventory menu; their positioning offset relative  to the entire menu, and spacing 
// between each option along both axes.
#macro	MENUINV_OPTION_XOFFSET			floor(display_get_gui_width() / 2) - 50
#macro	MENUINV_OPTION_YOFFSET			3
#macro	MENUINV_OPTION_XSPACING			50
#macro	MENUINV_OPTION_YSPACING			0

// Index values for the positions of the submenu references within the inventory's "menuRef" array. The final value is the sum of the number 
// of submenus which is also the length of the array the references reside in.
...
// Determines the characteristics of the opening animation for the inventory; consisting of shifting contents from off the top and bottom and
// the screen onto the screen while its opacity slowly increases to a value of 1.0 AKA fully opaque.
#macro	MENUINV_OANIM_ALPHA_SPEED		0.06
#macro	MENUINV_OANIM_MOVE_SPEED		0.25
#macro	MENUINV_OANIM_YTARGET			0.0

// Characteristic for the closing animation for the inventory. It's similar to the opening animation, but moves the elements off the top and 
// bottom of the screen linearly instead of the smooth decay of the opening animation's movement logic.
#macro	MENUINV_CANIM_ALPHA_SPEED		0.09
#macro	MENUINV_CANIM_YTARGET		   -20.0

// The macro for the unique key used to store the control icon group for the invenory's cursor movement/page shifting input information.
#macro	MENUINV_ICONUI_CTRL_GRP			"menuinv_move_icons"

// Macros for the characteristics of where the inventory's control group will be rendered onto the screen relative to the bottom-right of the
// GUI and the spacing between each icon/descriptor pairs.
#macro	MENUINV_CTRL_GRP_PADDING		2
#macro	MENUINV_CTRL_GRP_XOFFSET		5

// Each macro represents the index values where each of the two menu cursor movement icon/descriptor pairs and each of the two inventory page
// shift icon/descriptor pairs, respectively.
...
// The macro for the unique key used to store the control icon group for the inventory's selection/return/close input information.
#macro	MENUINV_ICONUI_CTRL_GRP2		"menuinv_selret_icons"

// Determines the position of this second control group on the screen, as well as the padding between each group's icon/descriptor pair.
#macro	MENUINV_CTRL_GRP2_PADDING		2

// Each macro represents the index values where the menu select icon/descriptor pair and menu return icon/descriptor pair, respectively.
#macro	MENUINV_CTRL_GRP2_SELECT		0
#macro	MENUINV_CTRL_GRP2_RETURN		1

// The opacity levels for the backgrounds that are blended together to create a nice blue/black alpha gradient effect across the screen.
...
function str_inventory_menu(_index) : str_base_menu(_index) constructor {
	...
	create_event = function(){
		...
		// Add "options" for the menu that are simply the names for each page of the inventory. Those pages themselves are unique menu 
		// instances that manage their own data and input independent of this one.
		add_option("Items");
		add_option("Notes");
		add_option("Map");
		...
		// Attempt to grab the control group that is utilized by the inventory menu. If it doesn't exist, the returned value is undefined and
		// then the group is created and stored in the local value.
		...
		with(CONTROL_UI_MANAGER){
			...
			// Create the control group at the desired position on the inventory menu, and store the reference it returns so the inventory 
			// can then store it for use when drawing the group. Then, add the desired elements to the group in question.
		...
		// After the required section has had its menu initialized, its control group information is grabbed if it already exists, or created
		// should the group not currently exists. It will store info for menu interaction inputs.
		...
		with(CONTROL_UI_MANAGER){
			...
			// Create the control group which is used in all inventory menu sections as they all use the Select/Close inputs.
	...
	/// Carry over the reference to the base struct's destroy event so it can be called through the inventory menu's destroy event function.
	...
	destroy_event = function(){
		...
		// Loop through and destroy any of the three submenus (Or all of them) that were created during this lifetime of the inventory menu.
	...
	draw_gui_event = function(_xView, _yView, _wView, _hView){
		// A local value that allows menu elements to slide off/on the bottom of the screen as other elements go off/on the otp of the screen
		// alongside the menu y position during the opening/closing animation, respectively. Multiplied by two to cancel out that upward 
		// sliding motion of the y value.
		var _yy = (y * 2.0);
		...
		// After the background elements have all been drawn, the inventory's section names will be drawn on the top portion of the menu that
		// is outside of the currently active section.
		...
		if (GAME_IS_TEXTBOX_OPEN){
			with(TEXTBOX){
				// Like within the textbox's drawing logic, ignore displaying control information when the textbox log is open, as that will 
				// handle drawing control information to the screen since it is currently being controlled by the user.
			...
			// Prevent drawing anything below this block of code so the inventory control information isn't drawn on top of the textbox/
			// textbox log's control information.
			return;
		}
	...
	initialize_submenu = function(_index){
		...
		// Attempt to create an instance for the new page of the menu. If an instance already exists somehow this function will return the 
		// value "noone" which will then cause the value to be grabbed from the menu's sInstance reference value.
		...
		// Store the references to each control group so they can be manipulated by the current section if that is needed given the current 
		// state of that section. Also store the alpha so it can be copied over since this function can be called both before and after the 
		// inventory's opening animation.
		var _movementCtrlGroup	= movementCtrlGroup;
		var _interactCtrlGroup	= interactCtrlGroup;
		var _alpha				= alpha;
		
		// Store the reference to the inventory's page and then set the flags that activate the menu as the current page. A reference to this
	...
	state_default = function(_delta){
		...
		// Only process this chunk of code if the inventory is currently allowed to close on detection of the return key being pressed. Since
		// the inventory only tracks two inputs for shifting between sections, the current section's return key input detection is used.
		...
		// Don't allow the inventory menu to switch pages (The "pages" in question are the item, note, and map sections, respectively) if its
		// flag to allow so is currently cleared.
		if (!MENUINV_CAN_CHANGE_PAGE)
			return;
		
		// Store the current option index before calling the function to possibly update the cursor's position. If the value happens to 
		// change, the check below will pass and the currently active submenu will be updated accordingly.
		...
		// Deactivate the previous inventory page/menu and the initialize the current one (If it hasn't already been initialized during this 
		// lifetime of the inventory) before activating it.
	...
	state_open_animation = function(_delta){
		...
		// managed by the current section will also have their positions updated.
		if (y != MENUINV_OANIM_YTARGET){
			y -= y * MENUINV_OANIM_MOVE_SPEED * _delta;
			
			// Double the inventory's current y position so these upper elements slide up from the bottom of the screen instead of sliding 
			// down alongside the upper elements as they slide down onto the screen from above it.
			var _y		= y * 2.0; 
			var _curY	= 0;
			...
			// The distance between the current y position and its target is close enough that the animation can finish its movement-based 
			// portion. As such, y is set to zero and positional updates will no longer occur.
			if (y > -_delta){
				y = MENUINV_OANIM_YTARGET;
				
				// Also set both control groups to their required target values in case they were some decimal value as y would be despite 
				// being higher than the current negative delta.
		...
		// Increase the visibility of the inventory by the desired speed. If the value goes above 1.0 it will be limited back to 1.0 in case 
		// it hits that value before the movement portion of the animation has completed.
		if (alpha < 1.0){

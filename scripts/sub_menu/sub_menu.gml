...
// Since sub menu instances default to a vertical orientation with a width of one option per row, this value is used to determine the spacing
// needed for said default orientation.
...
function str_sub_menu(_index) : str_base_menu(_index) constructor{
	...
	replace_options = function(_options, _width, _visibleWidth, _visibleHeight){
		...
		// Reset the menu such that it is highlighting the top-left option since the previous values are being replaced. Otherwise, these 
		// values could end up outside the new valid range of options.
		curOption		= 0;
		selOption		= -1;
		visibleAreaX	= 0;
		visibleAreaY	= 0;
		...
		// Loop through the existing option structs; replacing each one's previous oName value with the value found at the same index inside 
		// the _options array. Once the previous number of options is passed, new option structs will be created to store additional values.
		...
		// If the new set of options happens to be less than the previous size, the extra options will be removed from the menu until the 
		// amount matches the length of the _options array.
	...
	state_default = function(_delta){
		...
		// The sub menu detects the select input was released, so it should invoke its "option selected" state by copying the value of 
		// curOption into selOption. The sub menu's state is zeroed out after this.
		...
		// The sub menu detects the return input was released (If the necessary flag is set to allow that), so it should signal that the sub 
		// menu needs to be closed; be it through an animation or having it instantly disappeared depending on context.
		if (MENUSUB_CAN_CLOSE && MINPUT_IS_RETURN_RELEASED){
			flags = flags | MENUSUB_FLAG_CLOSING;
			object_set_state(0);
		}
		
		// No selection or return logic was processed, so another check is made to seeing if a cursor move needs to occur relative to the 
		// four inputs it utilizes, and if auto-scrolling is active or not.
...
function create_sub_menu(_menuToCreate, _parentMenu, _x, _y, _options, _width = 1, _visibleWidth = 1, _visibleHeight = 3, _font = fnt_small){
	...
	with(_submenuRef){
		...
		// Don't copy over anything from the _options argument if an array wasn't provided. The reference to the sub menu that was just 
		// created is simply returned instead.

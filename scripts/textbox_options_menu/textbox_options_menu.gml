...
function str_textbox_options_menu(_index) : str_sub_menu(_index) constructor {
	// Two values unique to the textbox options menu that will store the maximum possible extents of the options for the menu. Padding is then
	// applied on top of these values to create a background.
	contentWidth	= 0;
	contentHeight	= 0;
	
	/// @description
	///	Called during every frame that the menu exists for. It is responsible for rendering this menu's contents to the game's screen.
	///	
	...
	draw_gui_event = function(_xPos, _yPos, _wView, _hView, _shadowColor = COLOR_BLACK, _shadowAlpha = 1.0){
		// Draw the background for the menu in the same style as the textbox itself by borrowing its background. It should always be offset 
		// more on the left than on the right to make room for the cursor.
		...
		// Finally, use the default function for displaying a menu's visible region of options (In this case all options will be visible).

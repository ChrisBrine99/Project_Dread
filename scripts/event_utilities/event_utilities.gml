// Macros that determine the size of the buffer (which must be set in bytes) and that represents the total number of bits found within that 
// event flag buffer relative to the size in bytes, respectively.
...
// A global variable that will store a buffer of bits for various aspects of the game; from locks on doors to cutscenes, and so on. These 
// values can either be set or cleared to cause these unique events to happen or be removed from the game.
...
function event_set_flag(_flagID, _flagState){
	...
	// Grab the byte offset that will be used to read the desired group of bits from the buffer, and then calculate the offset into that byte
	// that the desired bit resides.
...
function event_get_flag(_flagID){
	...
	// Finally, return a bool that is either true (1) or false (0) depending on what is calculated from bitwise ANDing the grabbed bitfield 
	// against a value where only the desired bit is set.

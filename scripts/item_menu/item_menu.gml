...
// Macros for the bits within the item menu that are utilized for various non-state functionality. Each are unique to this menu.
...
// Since the positional offsets for the options and their spacing are constant in the item menu, they will be set using macros instead of 
// referencing their relevant variables within the menu, which should result in VERY slightly faster execution.
...
// Determines how fast the menu's currently highlighted item will move up along the y axis and the value that it will need to hit before the
// offset rolls back over to 0.
#macro	MENUITM_HLITEM_ANIM_SPEED		0.075
#macro	MENUITM_HLITEM_OFFSET_LIMIT		2.0

// The two variations on how displayed item quantities within the inventory can be formatted. The first is the default method for anything 
// with a stack limit greater than one, and the second is the amount of ammunition left in the weapon's current magazine (Melee weapons 
// aside from the chainsaw are excluded since they don't  have to use ammo).
#macro	MENUINM_QUANTITY_STANDARD		"x{0}"
#macro	MENUINM_QUANTITY_WEAPON			"[{0}]"

// The default string that will be used for an empty item slot when displayed on the UI for the player. The empty slot's info text will also
// use this default string.
#macro	MENUITM_DEFAULT_STRING			"---"

// A group of all the possible options an item can have access to whenever a given item is selected by the player within the item inventory.
// These strings are then used to determine what should be done upon selection within the item's options sub menu.
...
// The dimensions of the surface that the selected item's option menu will be rendered onto, so it can have a sliding animation that doesn't
// render outside of the dimensions of the menu after animations have completed.
#macro	SUBMENU_ITM_SURFACE_WIDTH		40
#macro	SUBMENU_ITM_SURFACE_HEIGHT		40

// Determines the two x positions the item's option menu text will shift between during the opening and closing animations for this sub menu;
// allowing it to slide on and off of the surface it will be drawn onto.
#macro	SUBMENU_ITM_ANIM_X1			   -50
#macro	SUBMENU_ITM_ANIM_X2				0

// Determines how fast the selected item's option menu will fade into and out of visibility during its opening and closing animations. It 
// also works as the value used to lerp the menu's options between the two x positions stated above.
...
function str_item_menu(_index) : str_base_menu(_index) constructor {
	// Create two arrays that will store data about the item inventory's contents so the relevant parts can be used and/or displayed on the 
	// UI for the player to see while accessing their items.
	...
	// Two variables that store references to menus. The first will hold the sub menu instance that is responsible for displaying and 
	...
	// Variables specific to the item options menu and how it is rendered within the item menu iteself. They are the surface that the sub 
	// menu is drawn onto (Its default rendering method is disabled to allow this), and the position of the sub menu relative to the overall
	// menu's position.
	itemOptionsSurf		= -1;
	itemOptionsMenuX	= 0;
	itemOptionsMenuY	= 0;
	...
	// Value that loops between zero and 2.0 to create a verticalbobbing motion for an item that is being highlighted by the menu's cursor.
	curOptionOffset		= 0.0;
	...
	create_event = function(){
		...
		// Set up the item menu to it has the same number of options as there are slots available in the player's item inventory. Using that
		// number, the base and option parameters are both initialized.
		...
		// Loop through the item inventory array so the menu can construct itself to accurately represent what is contained within that data.
		...
		for (var i = 0; i < _length; i++){
			...
			// Call the function to create the necessary data the menu will use to manipulate/render this item, and then add it as an option
			// to the menu using its name and description.
	...
	/// Store the pointer to the base menu's destroy event so it can be called within the item menu's version of the event alongside the 
	/// other logic it needs when cleaning itself up.
	__destroy_event = destroy_event;
	/// @description 
	///	Called whenever the item menu is closed by the player. It handles cleaning up memory that was allocated from the *str_base_menu*
	/// struct while also destroying the sub menu it manages if it exists during this lifetime of the item menu struct.
	destroy_event = function(){
		__destroy_event(); // Call parent destroy event to manage everything that was inherited.
		
		// Should either the surface for the selected item's option menu or that menu struct exist when the item inventory is being 
		// destroyed they will be tidied up here.
	...
	draw_gui_event = function(_xPos, _yPos, _wView, _hView){
		// Set the font used by all text within this menu, set the proper alignment for the quantity text that can appear alongside an 
		// item's icon, and set the opacity of all draw commands that don't take in an alpha level to the current opacity of the menu.
		draw_set_font(fnt_small);
		draw_set_halign(fa_right);
		draw_set_valign(fa_bottom);
		draw_set_alpha(alpha);
		
		// Increase the y offset of the currently highlighted item by its speed relative to the current delta time until it hits or exceeds 
		// its limit value; where it will wrap back around to 0.
		...
		// Loop through all available inventory slots to display them within the UI of the item menu. Occupied slots will contain some 
		// information about the item itself as well as a sprite that represents the item.
		...
		for (var yy = 0; yy < height; yy++){
			for (var xx = 0; xx < width; xx++){
				// Determine the option's index based on the current x/y position of the slot that will be drawn. Should this value exceed 
				// the inventory's current size, it will break out of this loop and the outer loop by setting yy to the menu's height.
				_option = (yy * width) + xx;
				if (_option >= _length){
					yy = height;
					break;
				...
				// Determine blending characteristics of the item slot/icon as well as the icon's positional offset based on whether the 
				// slot in question is visible (the "else" block), highlighted (_option == curOption), the primary selection (_option == 
				// selOption), of the stored selection (_option == auxSelOption), respectively.
				...
				// Determine whether to use the default version of the slot sprite or its highlighted counterpart depending on what the 
				// _slotCol variable was set to in the if/else block above.
				...
				if (_item != INV_EMPTY_SLOT){
					...
					// Displays the current quantity and the equip symbol if needed for the current item being rendered.
		...
		// Draw the currently highlighted item's name and description below the region of the item menu that contains the player's item 
		// inventory and its current contents.
		_optionX	= _xPos + optionX;
		...
		// Don't bother with any of the surface rendering/swapping logic below if the sub menu containing the selected item's options isn't
		// currently opened for the player to select from.
		...
		// Switch to rendering onto the surface for the selected item's option menu. Clear the surface to the color (0, 0, 0, 0) so the 
		// previous frame's rendering doesn't show up again creating a smear. Then, jump into scope of the menu in question and draw its 
	...
	/// @description 
	///	Allows a single line of code calling this function to encapsulate all that is required to reset the item menu back to its default 
	/// parameters before assigning its current state back to *state_default*.
	reset_to_default_state = function(){
		// Re-enable the change page and two additional cursor movement inputs within the movement control group so they are shown again.
	...
	/// @description 
	/// When called, this function will call the "item_inventory_add" function; passing in all of its parameters into that function. On top 
	/// of that, the renderable data for the item menu are updated as required if some amount of all of the item was actually added into 
	/// the item inventory.
	...
	add_item_to_inventory = function(_item, _amount, _durability = 0, _ammoIndex = 0){
		...
		// Since we don't know what slot the item was added into the inventory, all available slots are checked and when there is a 
		// difference in the slot's state (has item vs no item) and the current renderable information, call the function to create that 
		// info for the item menu.
		var _length = array_length(global.curItems);
		for (var i = 0; i < _length; i++){
	...
	/// @description 
	///	Creates a new struct within the *itemDataToRender* struct that stores the string versions of any values that need to be rendered 
	/// onto the item menu's inventory grid.
	...
	create_internal_item_data = function(_slot){
		...
		// Determine if the slot should display an "equipped" icon by seeing if the item occupying it is equipped in any of the player's 
	...
	/// @description 
	///	Removes whatever item is found within the provided slot index in the player's item inventory. Optionally, an amount can be provided 
	/// in case the entire stack doesn't need to be removed. Omitting this value will remove the entire stack found in that slot.
	...
	/// @description 
	/// Takes whatever was previously stored for renderable information within the provided slot index and either replaces it with new 
	/// information to reflect the item that is now within the slot or removes the information if no item occupies the slot.
	...
	refresh_internal_item_data = function(_slot, _itemRef){
		// By default, a quantity of zero (Excluding weapons as they can have a quantity of zero and still exist in the inventory) will 
		// signify the slot in question no longer contains an item, and the renderable data should be removed.
		var _quantity	= 0;
		var _invItemRef = global.curItems[_slot];
		if (_invItemRef != INV_EMPTY_SLOT)
			_quantity = _invItemRef.quantity;
		
		// If the quantity is greater than zero OR the item in question is a weapon, the branch below is taken and the contents of the 
		// renderable data are refreshed to reflect any potential changes.
		...
		// Check if there is an item occupying the current slot and whether or not that item is equipped. If both these checks are true, the
		// item in question is unequipped from the player before the renderable data for the item is removed.
		...
		// Remove the reference to the item as it is no longer within the item inventory. Then, remove the item's data from the menu option 
	...
	/// @description 
	///	The item menu's default state. It will handle selecting a slot within the menu, closing the menu by releasing the *auxilliary* 
	/// return input (The master inventory menu manages the standard return input), and moving the cursor if no option was selected and the 
	/// menu shouldn't close at the moment.
	///	@param {Real} delta		The difference in time between the execution of this frame and the last.
	state_default = function(_delta){
		// First, process player input for the current frame, and then check if they released the return key. If so, the item menu will 
		// signal to the inventory that it is time to play its closing animation.
		process_player_input();
		if (MINPUT_IS_AUX_RETURN_RELEASED){
			// Don't allow the first detection of the release input close the inventory if the value found in auxSelOption isn't default
			// value. If so, reset back to the item menu's default state.
			if (auxSelOption != -1){
		...
		// Like above for the auxillary return input, the item menu is reset back to its default state but when the normal return input for 
		// menus is pressed instead of that auxillary input. That input flag is then cleared so the inventory menu doesn't accidentally 
		// detect it and close the menu during the next frame since it is processed before this menu is.
		...
		// The select input was released, so an item will be selected for the player to interact with its list of available options.
		if (MINPUT_IS_SELECT_RELEASED && curOption != auxSelOption){
			// Don't jump into any of this selection code logic if the current slot is empty AND the inventory isn't currently set to move 
			// an item to another slot. If slot movement is occurring, the check is bypassed to be stopped further below.
			...
			// Disable the page and cursor left/right input information since the inventory is no longer able to switch between each of the
			// three sections contained within it.
			var _movementCtrlGroup = movementCtrlGroup;
			with(CONTROL_UI_MANAGER){
			...
			// Determine the options that the item inventory's sub menu--the item options menu--will have based on its given type ID. This 
			// will determine what can and cannot be done to the item.
			var _options	= -1;
			...
			switch(_itemType){
				...
				case ITEM_TYPE_WEAPON:
					...
					// An item that would normally have the "Equip" option as the first in the list of available options will have it
					// changed to "Unequip" if the weapon happens to be equipped to the player.
				...
				case ITEM_TYPE_KEY_ITEM:
					...
					// Add the "Use" and "Drop" options for key items here by checking if the flags for adding said options are set within 
					// the item's flag variable.
			...
			// If the struct already exists, replace the previous options and clear the bit that is normally set when the sub menu is 
			// closing. Finally, copy the vertical spacing between options like was done in below so the height of the menu's background 
			// can be calculated.
			var _ySpacing = 0;
			with(itemOptionsMenu){
			...
			// Create the sub menu for the select item's available options. Then, set the alpha to fully opaque since the item menu renders 
		...
		// After checking for the relevant non-cursor movements have been checked, the cursor will check if its position should be updated 
		// based on its "auto-shift" timer is that is active, or if a direction was pressed in all other instances.
		update_cursor_position(_delta);
	}
	
	/// @description 
	///	Handles the opening animation for the selected item's option window. Once the animation has finished, the submenu will begin to 
	/// check for input and then process whichever option is selected.
	///	@param {Real} delta		The difference in time between the execution of this frame and the last.
	state_open_item_options = function(_delta){
		// Jump into the submenu's scope and update the parameters involved in the animation. Once all values have reached their targets, 
		// flip the flag _isOpened to true to signify the animation is done and the item menu's state should change.
		var _isOpened			= false;
	...
	/// @description 
	///	Handles the closing animation for the selected item's option window. Once the animation has finished, standard input functionality 
	/// returns to the item inventory menu itself as it resets to its default state once again.
	///	@param {Real} delta		The difference in time between the execution of this frame and the last.
	state_close_item_options = function(_delta){
		// Jump into the select item's option menu submenu to update the parameters that are used in the closing animation. Once the 
		// conditions of the animation are met, the local _isClosed flag is set to true to signify the item menu can reset itself.
		var _isClosed			= false;
		var _itemOptionsAlpha	= itemOptionsAlpha;
		with(itemOptionsMenu){
			optionX				= lerp(SUBMENU_ITM_ANIM_X1, SUBMENU_ITM_ANIM_X2, _itemOptionsAlpha); 
		...
		// When flagged to close the item inventory menu in its whole on an item's use, this branch of the code is taken. It will also check
		// if the textbox needs to opened after the menu has finished its closing animation.
		...
		// When flagged to open the textbox (But not also flagged to close the item inventory menu), the menu will switch to a special 
		// state which it will remain in until the textbox that it was signaled to open is closed.
	...
	/// @description 
	///	The state the item menu is set to whenever its sub menu has been activated. It processes what should happen depending on what option
	/// within the sub menu is selected, or closing that sub menu if needed.
	///	@param {Real} delta		The difference in time between the execution of this frame and the last.
	state_navigating_item_options = function(_delta){
		// Grab the current value of selOption within the sub menu, and then check if it is currently closing or not. Both values are used 
		// below like a menu's select/return inputs are processed to determine what needs to be done within the menu to meet the required 
		// criteria for either input.
		var _selOption	= -1;
		...
		// An option in the sub menu was selected, so the selcted option is used to determine what needs to happen within the item menu 
		// since it has ties to the player's item inventory and the sub menu is a generic menu that doesn't handle its own selected option(s)
		// automatically.
		if (_selOption != -1){
			var _oName = "";
			with(itemOptionsMenu)
			...
			switch(_oName){
				...
				case MENUITM_OPTION_COMBINE:	// The item is being combined; store its slot index, and return to normal inventory function so another can be selected.
					...
					// Update the Control UI for the Item Menu so it displays the left and right cursor movement inputs alongside the up/
					// down inputs that were displayed for the selected item's option menu.
					var _movementCtrlGroup = movementCtrlGroup;
					with(CONTROL_UI_MANAGER){
						update_control_group_icon_active_state(_movementCtrlGroup, MENUINV_CTRL_GRP_CURSOR_LEFT,	true);
						update_control_group_icon_active_state(_movementCtrlGroup, MENUINV_CTRL_GRP_CURSOR_RIGHT,	true);
				...
				case MENUITM_OPTION_DROP:		// Removes slot from the inventory; creates a world item on the floor representing the slot's contents.
					...
					// After the item is unequipped, it will be removed from the inventory and have an instance of obj_world_item containing
					// the characteristics of what was in this slot.
					item_inventory_slot_create_item(PLAYER.x - 8, PLAYER.y - 8, selOption);
					
					// Remove the reference to the item as it is no longer within the item inventory. Then, remove the item's data from the 
					// menu option with the same slot index.
	...
	/// @description
	///	A single-frame state that attempts to use the select item and apply its effects to the player, game world, area, etc.. The item's 
	/// option window begins its closing animation after this.
	///	@param {Real} delta		The difference in time between the execution of this frame and the last.
	state_use_item = function(_delta){
		...
		// Set the flag within the item inventory if the item being used requires the item inventory menu to be closed when it is used by 
		// the player in-game.
		if ((_useFlags & USEITM_FLAG_CLOSE_MENU) != 0)
			flags = flags | MENUITM_FLAG_CLOSE_ON_USE;
			
		// Set the flag within the item menu that tells it to toggle the flag within the inventory menu struct (Which handles opening and 
		// closing this menu) that activates the textbox with whatever contents exists in the text queue.
	...
	/// @description 
	///	A single-frame state that attempts to equip the selected item to the player in one of their six slots for various types of equipment.
	/// The item's option window begins its closing animation after this.
	///	@param {Real} delta		The difference in time between the execution of this frame and the last.
	state_equip_item = function(_delta){
		...
		// If there was a previously equipped item in the slot where this new item is equipped, its flag will be set to false before flipping
		// the equipped flag for the new item to true.
	...
	/// @description 
	///	A single-frame state that attempts to unequip the item in the slot stored in the "selOption" variable. This will call the appropriate
	/// function for the type of item being unequipped, and then the item's option menu window will begin its closing animation right after.
	///	@param {Real} delta		The difference in time between the execution of this frame and the last.
	...
	/// @description 
	///	The state that is executed when the user has selected two different occupied slots in their inventory. It will take these items and 
	/// either combine as much of them into a single slot as possible (Happens when both items match in ID/name) or it will attempt to see 
	/// if they can combine together to result in a new item that is added to the inventory. After running this state once, the inventory 
	/// will begin returning itself back to its default state.
	///	
	///	@param {Real} delta		The difference in time between the execution of this frame and the last.
	state_combine_item = function(_delta){
		...
		// If the item IDs match, the first item will have its current quantity combined into the second item's slot if possible. Any 
		// remainder that exceeds the stack limit for the item will remain in the first slot.
		var _firstItemID	= _firstItem.itemID;
		var _secondItemID	= _secondItem.itemID;
		if (_firstItemID == _secondItemID){
			item_inventory_slot_combine(auxSelOption, selOption);
			refresh_internal_item_data(auxSelOption);
			refresh_internal_item_data(selOption);
			reset_to_default_state();
			return;
		}
		
		// Check if the second item has the smaller ID value of the two. If so, a swap of specific local values will need to occur before
		// any combination logic can be executed, as that is optimal for how the combo data itself is organized.
		if (_firstItemID > _secondItemID){
			// Swap the references to the items since the higher of the items via their ID should always be the second item in the combo
			// process. Store the first refernce in a temporary value so it isn't overwritten during the swapping process.
			var _tempItem	= _firstItem;
			_firstItem		= _secondItem;
			_secondItem 	= _tempItem;
			
			// Create a temporary value to store the first item's ID before overwriting it with the second item's ID as they have switched
			// places to simplify the code below this. Set the second item's ID to the temporary value storing what the first ID was.
			var _tempID		= _firstItemID;
			_firstItemID	= _secondItemID;
			_secondItemID	= _tempID;
			
			// Finally, switch the values stored within _firstSlot and _secondSLot so they match now that the items have swapped.
			_firstSlot		= selOption;
			_secondSlot		= auxSelOption;
		}
		reset_to_default_state();
		
		// Check if valid combo data exists for the two items being combined by the player. If not, the function returns early so none of
		// the combo-specific logic is ran. Then, the textbox state is entered as text is displayed for a failed combination.
		...
		// Copy all of the relevant combo data into local values so they will be faster to access throughout the combination process.
		var _firstCost		= 0;
		var _secondCost		= 0;
		var _resultItem		= "";
		var _resultMin		= 0;
		...
		// UNIQUE CASE --- If the second item happens to be upgrade parts (As a key item it will always have a higher ID value compared to
		// the items it can upgrade), a slightly different variant of the combination process will occur where the item being upgraded
		// retains its original quantity and contained ammunition values.
		if (_secondItem.itemName == ITEM_UPGRADE_PARTS){
			// Store the original quantity and current ammunition for the item before it is removed from the inventory.
			var _quantity	= 0;
			var _ammoIndex	= 0;
			...
			// Remove the contents of the weapon from the inventory, and remove a single upgrade part from the inventory (Its amount of 1
			// is specified in case the stack limit for the upgrade parts ever changes since all upgrades use a single upgrade part). 
			remove_item_from_slot(_firstSlot);
			remove_item_from_slot(_secondSlot, 1);
			
			// Check if the item being combined has a durability value associated with it (Ex. all flashlights exclude a durability value
			// form their item data since they don't use it). If there is a value, grab it as the upgraded item will always start at its
			// maximum possible durability. Then, use all the stored values to add the upgraded item to the inventory.
			var _durability	= 0;
		...
		// Check if the current amount contained in the first slot and second slot meet what is needed for the combo to work. If not, pop
		// up a textbox to let the player know they don't have enough to complete the recipe and exit the combination process attempt.
		...
		// Remove the required amount for the combo from each slot involved in the process.
		remove_item_from_slot(_firstSlot, _firstCost);
		remove_item_from_slot(_secondSlot, _secondCost);
		
		// Rendomly determine an integer value between the minimum and maximum possible if the values differ. If not, the maximum is used.
		var _quantity = (_resultMin != _resultMax) ? irandom_range(_resultMin, _resultMax) : _resultMax;

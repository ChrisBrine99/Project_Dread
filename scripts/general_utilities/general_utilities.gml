...
// The value that equates to one second of real-time in the game's units. An example would be an entity with a speed value of 1.0 would move
// roughly 60 pixels per second.
#macro	GAME_TARGET_FPS					60.0

// The maximum possible delta that the game can have; equivalent to roughly 10 frames per second. Anything lower than that will have the 
// game slow down to prevent any glitches or bugs because of a massive delta value.
...
// A catchall specifically for when a key item doesn't have a proper function assigned to it due to malformed data being found within the 
// item's data upon it being loaded.
#macro	NO_FUNCTION					   -64

// Macros for characters that are used throughout the games code for various purposes; from parsing data for items when they're loaded into 
// the game, to punctuation pauses during the textbox's "typing" animation, and so on.
#macro	CHAR_SPACE						" "
#macro	CHAR_COMMA						","
#macro	CHAR_COLON						":"
...
// Macros for how color data should be formatted within a string of text. It must be 0xBBGGRR otherwise the value is ignored and defaulted 
// to the macro COLOR_DARK_GRAY.
...
// Macros that explain what each tile index in the floor materials tileset refers to within the player's step sound effect logic, so it will
// know the proper sound to play relative to the material they are moving on.
...
function sound_effect_play(_sound, _soundType = STNG_AUDIO_GAME_SOUNDS, _volume = 1.0, _pitch = 1.0, _priority = 0, _stopPrevious = false, _loops = false, _offset = 0.0){
	...
	// Adjust the volume of the sound absed on the current master and current volume for the sound's type. Then, play the desired sound 
	// effect and return its ID.
...
function string_split_lines(_string, _font, _maxWidth, _maxLines = 1){
	...
	// Loop through the string that was passed into the function until it is considered formatted by the parameters specified upon the 
	// function call.
	...
	for (var i = 1; i <= _numChars; i++){ // Starts from one since GML's font functions are indexed starting at one.
		...
		// A space character has been found OR the current character is the final character in the unformatted version of the string, a 
		// check occurs to see if the word parsed can fit on the current line or a new line needs to be added for it. If that new line 
		// exceeds the max number of lines allowed, it is ignored and whatever is formatted already is returned.
		if (_curChar == CHAR_SPACE || i == _numChars){
			...
			if (_lineWidth + _wordWidth + _spaceWidth > _maxWidth){
				...
				// After handling the potential extra space at the end of the line, add it to the formatted with a newline character where 
				// the space used to be (If there was one). Then, set up a new line of text to be parsed.
			...
			// No newline character required, add the string to the end of the line alongside the current word. Update the width of the 
			// line to match these newly added characters.
			_lineWidth += _wordWidth + _spaceWidth;
			_curLine   += _curWord + _curChar;
			_curWord	= "";
			continue;
		...
		// A newline character has been found within the string already, so it will add that line to the new string and a new line will 
		// begin. If adding this line would exceed the max number of lines, the line is ignored and the loop exits early.
	...
	// Finally, return the string that has been formatted with newline characters and clipped such that only the set number of lines exist 
	// within that string. If the final line wasn't placed into the string in the loop, it will be concatenated with the rest of the string.
	// Otherwise, it is already in the string so just "_newString" is returned by itself.
	if (_curLine != "")
...
function string_parse_color_data(_string){
	...
	// The characters @, {, and } are removed from the string and the remaining hex code is formatted into a number and stored in a list 
	// containing all the text colors and the region they will affect when drawn.
	...
	while(true){ // This is the dumbest way I could've made a loop but it works...
		...
		// Attempt to grad the hex value within the text. The length of this code is checked in case it exists malformed at the very end of 
		// the string (Ex. @0xFF and nothing else or something like that) and said length is checked to see if its a valid length for a 
		// color hex code (0xBBGGRR) or not along with checks to see if the code is malformed.
		...
		// Delete the hex code and the trio of characters--@, {, and }--that have to exist alongside the string so there is a known region 
		// for the different colored text. If the } character doesn't exist, it is assumed the rest of the string will be colored relative 
		// to the code that was parsed.
		...
		// Finally, add the parsed data into a struck that will hold the information of the color to use, the first character to use it on,
		// and the final character to use it on.
	...
	// The list and formatted string are returned in a simple struct that can then be utilized to create multi-colored text in various areas
	// of the game's GUI.
